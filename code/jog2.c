...
enum commands jog2_state( void )
{
    ...
    switch(msg)
    {
      case j_null:
        break;
      case jog_spin_left_fast:
        command = spin_left_fast;
        break;
      case jog_spin_right_fast:
        command = spin_right_fast;
        break;
      case done:
        jog_count++;
        printf("\t...Jog2 sequence completed %i\n\r", jog_count);
		if (DataRdyUSART( )) 
        {
            data_rec = ReadUSART ();
            ...
            else if (data_rec == '1') command = jog1;
        }    
        break;
      default:
		command = menu_system;
        printf("\t...bug: from jog2()\r\n");
        break;        
    }
    return (command);    
}
...
static enum jog_msg begin_state0(void)
{
    ...
    if (cycle >= 2u)
    {
        ...
        msg = jog_spin_left_fast;  // spin_left();
    }    
    return (msg);
}  
